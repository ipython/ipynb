...
class Code(NotebookExporter, Compiler):
    ...
    >>> assert type(Code().from_filename('exporter.ipynb')) is NotebookNode"""
...
class AST(Code):
    ...
    >>> assert type(AST().from_filename('exporter.ipynb')) is ast.Module"""
...
class Compile(AST):
    ...
    >>> assert Compile().from_filename('exporter.ipynb')"""
